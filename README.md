...
- 職責：分析全場局勢（敵我兵力比、產能差距、最強威脅）。
- 動態狀態機：根據局勢自動切換模式：
> EARLY_EXPAND：開局期，積極搶佔中立資源與工廠。
> DOMINATING：優勢期，採取壓迫式打法，滾雪球擴大優勢。
> SURVIVAL：劣勢期，龜縮防守，專注升級工廠與防禦，等待反擊機會。

- 影響：動態調整 AIProfile 中的全域權重（例如：劣勢時大幅提升 weight_defense）。

2. 戰術層 (Tactical Layer)：AIBrain & Evaluators
- 職責：具體的行動評估。由四個子模組組成：
- Combat (戰鬥)：評估單體進攻收益、防守急迫度、以及發起 協同集結 (Swarm) 攻擊。
- Economy (經濟)：計算地基變身與建築升級的投資報酬率 (ROI)，考量戰略價值與危險係數。
- Logistics (後勤)：管理兵力調度（前線補給 Pull、後方溢出轉移 Push）。
- Lab (科技)：決策技能施放時機（如：敵方大軍壓境時使用冰凍、對高價值目標施放虛弱）。
- 路徑分析：引入 PathAnalyzer 預估路徑上的潛在戰損，避免自殺式攻擊。

3. 執行層 (Execution Layer)：AIExecutor
- 職責：將抽象指令轉換為遊戲內的具體操作。
- 擬人化操作：引入 action_interval (APM限制) 與隨機波動，模擬人類玩家的操作延遲與注意力限制。
...
# 📝 學習與決策重點
- 架構設計：採用 管理器模式 (Manager Pattern)，將 Input、Render、Logic 切分為不同模組，降低耦合度。
- 效能優化：識別 Python 運算瓶頸，引入 C 語言處理 A* 尋路算法，理解跨語言介接 (FFI) 原理。
- 工程實務：資源路徑抽象化、JSON 設定檔管理、資料持久化 (Persistence)。
- 演算法應用：A* 路徑規劃、網格分區，碰撞檢測優化、Z-Buffer 渲染排序。
